...
RED = (255, 0, 0)
BLACK = (0, 0, 0)
OBSTACLE_COLOR = (79, 92, 105)  # Green color for obstacles
...
car_image = pygame.image.load("bike.png")
...
# Create a font for the "Game Over" message and score
...
# Define a button for returning to the main menu on the game over screen
main_menu_button_rect = pygame.Rect(WIDTH // 2 - 100, HEIGHT // 2 + 100, 200, 50)
...
while True:
        ...
        if event.type == pygame.QUIT:
            ...
            if game_over:
                if main_menu_button_rect.collidepoint(event.pos):
                    ...
                    car_x = WIDTH // 2 - car_width // 2  # Reset car position
                    car_x_velocity = 0  # Reset car velocity
    ...
    if game_over:
        ...
        # Draw "Main Menu" button on the game over screen
        main_menu_button_color = RED if main_menu_button_rect.collidepoint(pygame.mouse.get_pos()) else WHITE
        pygame.draw.rect(screen, main_menu_button_color, main_menu_button_rect)
        main_menu_text = font_difficulty.render("Main Menu", True, BLACK)
